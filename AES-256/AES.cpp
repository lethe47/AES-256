...
string encrypt(string data_)
{
    string data = padding(data_);
    const char* h = data.data();
    char* mingwen = nullptr;
    mingwen = const_cast<char*>(h);
    char miwen_hex[1024];
    unsigned char key[] = "BF3D6A0B97E473BE03153689E24A075B";
    AES aes(key);
    aes.Cipher(mingwen, miwen_hex);
    string data_miwen = miwen_hex;
    return data_miwen;
}
...
{
    const char* h = data.data();
    char* miwen_hex = nullptr;
    miwen_hex = const_cast<char*>(h);
    char result[1024];
    unsigned char key[] = "BF3D6A0B97E473BE03153689E24A075B";
    AES aes(key);
    aes.InvCipher(miwen_hex, result);
    string data_mingwen = result;
    return un_padding(data_mingwen);
}

int  convert_10_to_16(int num)
{
    switch (num)
    {
    case 1:
...
string padding(string in_str)
{
    int length = in_str.length();
    if (length >= 16)
        return in_str;
    int remain = 16 - length;
    int value = convert_10_to_16(remain);
    string pad = "abcdefghijklmnop";
    for (int i = 0; i < 16; i++)
    {
        if (i < length)
            pad[i] = in_str[i];
...
string un_padding(string in_str)
{
    int length = in_str.length();
    if (length > 16)
        return in_str;
    if (length == 16)
    {
        int value = convert_16_to_10(in_str[15]);//缺失长度
        if (value == 1000)
            return in_str;
        else
        {
            int len = 16 - value;
            return in_str.erase(len, 16);
        }
    }
    else
        cerr << "padding错误" << endl;
